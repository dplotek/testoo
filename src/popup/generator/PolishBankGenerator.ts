...
export interface PolishBankGeneratorOptions {
    bank?: number|undefined,
}

export function getValue(options: PolishBankGeneratorOptions): string {
    ...
    if (options.bank) {
        const bank = Banks.find(b => b.id === options.bank) || null;
    ...
    if (bankId.length === 0) {
        bankId = Banks[randomNumber(0, Banks.length - 1)].id.toString();
    }
