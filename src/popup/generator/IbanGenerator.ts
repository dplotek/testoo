...
import PolishBankGenerator from "./PolishBankGenerator.js";

export interface IbanGeneratorOptions {
    countryCode: string|undefined,
}

export default class IbanGenerator extends Generator {
    getValue(options: IbanGeneratorOptions): string {
        let value, country, countryIndex, bankId = '',
            bban, ctrlSum;

        if (options.hasOwnProperty('countryCode')
            && (countryIndex = IbanGenerator.Countries.findIndex(c => c.code === options.countryCode)) >= 0) {
            country = IbanGenerator.Countries[countryIndex];
        } else {
            country = IbanGenerator.Countries[this.randomNumber(0, IbanGenerator.Countries.length - 1)];
        }

        if (country.code === 'PL') {
            const generator = new PolishBankGenerator();
            bankId = generator.getValue(options);
        } else {
            bankId = this.randomNumber(0, 99999999).toString();
        }

        bban = bankId.toString();

        for (let i = bban.length; i < country.length - 4; i++) {
            bban += this.randomNumber(0, 9).toString();
        }

        value = bban + (country.code.charCodeAt(0) - 55) + (country.code.charCodeAt(1) - 55) + '00';
        ctrlSum = (98 - this.modulo(value, 97)).toString().padStart(2, '0');
        return country.code + ctrlSum + bban;
    }

    modulo(number: string, m: number): number {
        let result = 0;
        let x = '';
    ...
    static Countries: Array<{code: string, name: string, length: number}> = [
