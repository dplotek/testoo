...
export function getValue(options: PeselGeneratorOptions): string {
    ...
    if (options.startDate) {
        const validDate = getValidateDate(options.startDate);
    ...
    if (options.endDate) {
        const validDate = getValidateDate(options.endDate);
    ...
    result += randomNumber(0, 999).toString().padStart(3, '0');
    result += sexDigits[randomNumber(0, sexDigits.length - 1)].toString();
...
function getValidateDate(date: string): Date | null {
...
function getRandomDate(startDate: Date, endDate: Date): { year: number, month: number, day: number } {
...
function getNumberOfDays(year: number, month: number): number {
    let result = NumberOfDaysInMonth[month - 1];
